...
#include "SharedMemSegment.hpp"
#include "RobustExclusiveLock.hpp"
#include "RobustSharedLock.hpp"
#include "SharedMemWatchdog.hpp"

#include "MultiProducerConsumerRingBuffer.hpp"
...
public:
        ...
        : domain_name_(domain_name)
    {
        ...
        {
            std::cout << domain_name << " too long for domain name (max " << std::to_string(MAX_DOMAIN_NAME_LENGTH) << " characters" << std::endl;
        }
    }
                ...
                while (port_it != watched_ports_.end())
                {
                    ...
                    if (timeout_elapsed(now, *(*port_it)))
                    {
                        ...
                        catch (std::exception& e)
                        {
                            ...
                            std::cout << "Port " << (*port_it)->node->port_id << ": " << e.what() << std::endl;
    ...
    public:
        ...
        ~Port()
        {
            ...
            if (node_->ref_counter.fetch_sub(1) == 1)
            {
                ...
                catch (const std::exception& e)
                {
                    ...
                    std::cout << segment_name.c_str() << e.what() << std::endl;;
                }
            }
        }
                ...
                bool* listeners_active)
        {
            ...
            if (!node_->is_port_ok)
            {
                printf("the port is marked as not ok!");
            }
...
private:
            ...
            std::shared_ptr<Port> regenerating_port)
    {
        ...
        std::shared_ptr<Port> port; //what is being returned

        auto port_segment_name = domain_name_ + "_port" + std::to_string(port_id); //IMPORTANT, port identifier needed to open a port

        std::cout << "Opening " << port_segment_name << std::endl;

        std::unique_ptr<SharedMemSegment::named_mutex> port_mutex =
                SharedMemSegment::open_or_create_and_lock_named_mutex(port_segment_name + "_mutex");    //get the mutex

        std::unique_lock<SharedMemSegment::named_mutex> port_lock(*port_mutex, std::adopt_lock);    //sure

        if (regenerating_port)                                                      //not important
        {
            try
            {
            ...
            {
                std::cout << "Port " << port_id << " failed unlock_read_locks " << e.what() << std::endl;
            }
        }
        ...
        try
        {
            ...
            {
                std::cout << "Port " << port_id << " Zombie. Reset the port" << std::endl;
            ...
            // Try to open, Important: opens the port segment
            ...
            try
            {
                ...
                port_node = port_segment->get().find<PortNode>(
                    ("port_node_abi" + std::to_string(CURRENT_ABI_VERSION)).c_str()).first; //gets the PortNode

                if (port_node)
                {
                    port = std::make_shared<Port>(std::move(port_segment), port_node); //port is the object/pointer being returned
                }
                else
                {
            ...
            {
                std::cout << "Port " << port_id << " Couldn't find port_node " << std::endl;

                SharedMemSegment::remove(port_segment_name.c_str());

                std::cout << "Port " << port_id << " Removed." << std::endl;
            ...
            try
            {
                if (open_mode == Port::OpenMode::ReadExclusive) //this might be an issue
                {
                    try
                    {
                ...
                if (port)
                {
                    ...
                    std::cout << "Port " << port_node->port_id << " (" << port_node->uuid.to_string() << ") Opened" << Port::open_mode_to_string(open_mode) << std::endl;
                }
            }
            catch (std::exception&)
            {
                ...
                std::cout << "Existing Port " << port_id << " (" << port_uuid << ") NOT Healthy." << std::endl;

                SharedMemSegment::remove(port_segment_name.c_str());

                std::cout << "Port " << port_id << " (" << port_uuid << ") Removed." << std::endl;
        ...
        catch (std::exception&)
        {
            ...
            {
                std::cout << "Failed to create port segment " << port_segment_name << ": " << e.what() << std::endl;
            }
            ...
            if (port_segment)
            {
                ...
                catch (std::exception& e)
                {
                    ...
                    std::cout << "Failed init_port " << port_segment_name << ": " << e.what() << std::endl;
            ...
            uint32_t healthy_check_timeout_ms)
    {
        ...
        std::cout << "Port " << port_node->port_id << " (" << port_node->uuid.to_string() << Port::open_mode_to_string(open_mode) << ") Created." << std::endl;
